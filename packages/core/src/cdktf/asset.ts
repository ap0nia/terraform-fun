...
export interface TerraformAssetConfig {
  ...
  /**
   * Options for zipping the asset.
   * Since this has to be eval-ed to synchronously zip a file, 
   * it cannot use complex data types.
   */
  readonly zipOptions?: ZipOptions;
}
...
export class TerraformAsset extends Construct {
  ...
  constructor(scope: Construct, id: string, config: TerraformAssetConfig) {
    ...
    this.zipOptions = config.zipOptions ?? {};
  ...
  private _onSynthesize(session: ISynthesisSession) {
    ...
    switch (this.type) {
      ...
      case AssetType.ARCHIVE:
        const projectRoot = getProjectDirectory(process.cwd())
        const zipSyncPath = path.resolve(projectRoot, "scripts/zip.js");
        const command = `node "${zipSyncPath}" "${this.sourcePath}" "${targetPath}" "${JSON.stringify(this.zipOptions)}}"`
        execSync(command);
        break;
      default:
